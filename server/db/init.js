...
export function initDatabase(dbPath) {
  ...
  const createTables = `
    ...
    CREATE TABLE IF NOT EXISTS reviews (
      ...
      filling_score REAL CHECK(filling_score >= 0.5 AND filling_score <= 5 AND (filling_score * 2) = CAST(filling_score * 2 AS INTEGER)),
      pastry_score REAL CHECK(pastry_score >= 0.5 AND pastry_score <= 5 AND (pastry_score * 2) = CAST(pastry_score * 2 AS INTEGER)),
      appearance_score REAL CHECK(appearance_score >= 0.5 AND appearance_score <= 5 AND (appearance_score * 2) = CAST(appearance_score * 2 AS INTEGER)),
      overall_score REAL CHECK(overall_score >= 0.5 AND overall_score <= 5 AND (overall_score * 2) = CAST(overall_score * 2 AS INTEGER)),
      value_for_money_score REAL CHECK(value_for_money_score >= 0.5 AND value_for_money_score <= 5 AND (value_for_money_score * 2) = CAST(value_for_money_score * 2 AS INTEGER)),
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
